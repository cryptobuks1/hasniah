...
if (!function_exists('plumbing_parts_sc_emailer')) {	
	function plumbing_parts_sc_emailer($atts, $content = null) {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
				...
				. '<button href="#" class="sc_emailer_button" ' . (!empty($privacy) ? ' disabled="disabled"' : '') . '" title="'.esc_attr__('Submit', 'plumbing-parts').'" data-group="'.esc_attr($group ? $group : esc_html__('E-mailer subscription', 'plumbing-parts')).'">subscribe</button>'
