...
if ( !function_exists( 'plumbing_parts_register_theme_sidebars' ) ) {
    add_action('widgets_init', 'plumbing_parts_register_theme_sidebars');
...
if ( !function_exists( 'plumbing_parts_core_frontend_scripts' ) ) {
	function plumbing_parts_core_frontend_scripts() {
		...
		wp_enqueue_script( 'plumbing_parts-core-modernizr-script', plumbing_parts_get_file_url('js/photostack/modernizr.min.js'), array(), null, false );
		...
		if (is_array($theme_fonts) && count($theme_fonts) > 0) {
			...
			foreach ($theme_fonts as $font=>$v) {
				if (isset($fonts[$font])) {
					...
					if (!empty($fonts[$font]['css'])) {
						$css = $fonts[$font]['css'];
						wp_enqueue_style( 'plumbing_parts-font-'.str_replace(' ', '-', $font_name).'-style', $css, array(), null );
					} else {
			...
			if ($google_fonts)
				wp_enqueue_style( 'plumbing_parts-font-google_fonts-style', plumbing_parts_get_protocol() . '://fonts.googleapis.com/css?family=' . $google_fonts . '&subset=' . plumbing_parts_get_theme_option('fonts_subset'), array(), null );
		}
		
		// Fontello styles must be loaded before main stylesheet
		wp_enqueue_style( 'plumbing_parts-fontello-style',  plumbing_parts_get_file_url('css/fontello/css/fontello.css'),  array(), null);
		//wp_enqueue_style( 'plumbing_parts-fontello-animation-style', plumbing_parts_get_file_url('css/fontello/css/animation.css'), array(), null);

		// Main stylesheet
		wp_enqueue_style( 'plumbing_parts-main-style', get_stylesheet_uri(), array(), null );
		...
		if (plumbing_parts_get_theme_option('css_animation')=='yes' && (plumbing_parts_get_theme_option('animation_on_mobile')=='yes' || !wp_is_mobile()) && !plumbing_parts_vc_is_frontend())
			wp_enqueue_style( 'plumbing_parts-animation-style',	plumbing_parts_get_file_url('css/core.animation.css'), array(), null );
		...
		if (plumbing_parts_get_theme_option('responsive_layouts') == 'yes') {
			...
			wp_enqueue_style( 'plumbing_parts-responsive-style', plumbing_parts_get_file_url('css/responsive'.($suffix).'.css'), array(), null );
		...
		wp_enqueue_script( 'superfish', plumbing_parts_get_file_url('js/superfish.js'), array('jquery'), null, true );
		if (plumbing_parts_get_theme_option('menu_slider')=='yes') {
			wp_enqueue_script( 'plumbing_parts-slidemenu-script', plumbing_parts_get_file_url('js/jquery.slidemenu.js'), array('jquery'), null, true );
			//wp_enqueue_script( 'plumbing_parts-jquery-easing-script', plumbing_parts_get_file_url('js/jquery.easing.js'), array('jquery'), null, true );
		}

		if ( is_single() && plumbing_parts_get_custom_option('show_reviews')=='yes' ) {
			wp_enqueue_script( 'plumbing_parts-core-reviews-script', plumbing_parts_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
		}

		wp_enqueue_script( 'plumbing_parts-core-utils-script',	plumbing_parts_get_file_url('js/core.utils.js'), array('jquery'), null, true );
		wp_enqueue_script( 'plumbing_parts-core-init-script',	plumbing_parts_get_file_url('js/core.init.js'), array('jquery'), null, true );
		wp_enqueue_script( 'plumbing_parts-theme-init-script',	plumbing_parts_get_file_url('js/theme.init.js'), array('jquery'), null, true );
		...
		if (plumbing_parts_get_custom_option('show_video_bg') == 'yes' && plumbing_parts_get_custom_option('video_bg_youtube_code') != '') {
			wp_enqueue_script( 'plumbing_parts-video-bg-script', plumbing_parts_get_file_url('js/jquery.tubular.1.0.js'), array('jquery'), null, true );
		}

		// Google map
        if ( plumbing_parts_get_custom_option('show_googlemap')=='yes' ) {
            $api_key = plumbing_parts_get_theme_option('api_google');
            wp_enqueue_script( 'googlemap', plumbing_parts_get_protocol().'://maps.google.com/maps/api/js'.($api_key ? '?key='.$api_key : ''), array(), null, true );
            wp_enqueue_script( 'plumbing_parts-googlemap-script', plumbing_parts_get_file_url('js/core.googlemap.js'), array(), null, true );
        }

			
		// Social share buttons
		if (is_singular() && !plumbing_parts_storage_get('blog_streampage') && plumbing_parts_get_custom_option('show_share')!='hide') {
			wp_enqueue_script( 'plumbing_parts-social-share-script', plumbing_parts_get_file_url('js/social/social-share.js'), array('jquery'), null, true );
		}
		...
		if (plumbing_parts_get_theme_option('show_theme_customizer') == 'yes') {
			if (file_exists(plumbing_parts_get_file_dir('core/core.customizer/front.customizer.css')))
				wp_enqueue_style(  'plumbing_parts-customizer-style',  plumbing_parts_get_file_url('core/core.customizer/front.customizer.css'), array(), null );
			if (file_exists(plumbing_parts_get_file_dir('core/core.customizer/front.customizer.js')))
				wp_enqueue_script( 'plumbing_parts-customizer-script', plumbing_parts_get_file_url('core/core.customizer/front.customizer.js'), array(), null, true );
		}
		
		//Debug utils
		if (plumbing_parts_get_theme_option('debug_mode')=='yes') {
			wp_enqueue_script( 'plumbing_parts-core-debug-script', plumbing_parts_get_file_url('js/core.debug.js'), array(), null, true );
		}
...
if ( !function_exists( 'plumbing_parts_enqueue_slider' ) ) {
	function plumbing_parts_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			wp_enqueue_style(  'plumbing_parts-swiperslider-style', 			plumbing_parts_get_file_url('js/swiper/swiper.css'), array(), null );
			wp_enqueue_script( 'plumbing_parts-swiperslider-script', 			plumbing_parts_get_file_url('js/swiper/swiper.js'), array(), null, true );
			// jQuery version conflict with Revolution Slider
			//wp_enqueue_script( 'plumbing_parts-swiperslider-script', 			plumbing_parts_get_file_url('js/swiper/swiper.jquery.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'plumbing_parts_enqueue_polaroid' ) ) {
	function plumbing_parts_enqueue_polaroid() {
		wp_enqueue_style(  'plumbing_parts-polaroid-style', 	plumbing_parts_get_file_url('js/photostack/component.css'), array(), null );
		wp_enqueue_script( 'plumbing_parts-classie-script',		plumbing_parts_get_file_url('js/photostack/classie.js'), array(), null, true );
		wp_enqueue_script( 'plumbing_parts-polaroid-script',	plumbing_parts_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'plumbing_parts_enqueue_messages' ) ) {
	function plumbing_parts_enqueue_messages() {
		wp_enqueue_style(  'plumbing_parts-messages-style',		plumbing_parts_get_file_url('js/core.messages/core.messages.css'), array(), null );
		wp_enqueue_script( 'plumbing_parts-messages-script',	plumbing_parts_get_file_url('js/core.messages/core.messages.js'),  array('jquery'), null, true );
	}
}
...
if ( !function_exists( 'plumbing_parts_enqueue_portfolio' ) ) {
	function plumbing_parts_enqueue_portfolio($hover='') {
		wp_enqueue_style( 'plumbing_parts-portfolio-style',  plumbing_parts_get_file_url('css/core.portfolio.css'), array(), null );
		if (plumbing_parts_strpos($hover, 'effect_dir')!==false)
			wp_enqueue_script( 'hoverdir', plumbing_parts_get_file_url('js/hover/jquery.hoverdir.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'plumbing_parts_enqueue_diagram' ) ) {
	function plumbing_parts_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') wp_enqueue_script( 'plumbing_parts-diagram-chart-script',	plumbing_parts_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') wp_enqueue_script( 'plumbing_parts-diagram-raphael-script',	plumbing_parts_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if ( !function_exists( 'plumbing_parts_enqueue_popup' ) ) {
	function plumbing_parts_enqueue_popup($engine='') {
		if ($engine=='pretty' || (empty($engine) && plumbing_parts_get_theme_option('popup_engine')=='pretty')) {
			wp_enqueue_style(  'plumbing_parts-prettyphoto-style',	plumbing_parts_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			wp_enqueue_script( 'plumbing_parts-prettyphoto-script',	plumbing_parts_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else if ($engine=='magnific' || (empty($engine) && plumbing_parts_get_theme_option('popup_engine')=='magnific')) {
			wp_enqueue_style(  'plumbing_parts-magnific-style',	plumbing_parts_get_file_url('js/magnific/magnific-popup.css'), array(), null );
			wp_enqueue_script( 'plumbing_parts-magnific-script',plumbing_parts_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
...
if ( !function_exists( 'plumbing_parts_enqueue_custom_styles' ) ) {
	function plumbing_parts_enqueue_custom_styles() {
		...
		wp_enqueue_style( 'plumbing_parts-custom-style', $custom_css ? $custom_css : plumbing_parts_get_file_url('css/custom-style.css'), array(), null );
