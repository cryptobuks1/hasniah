...
if ( !function_exists( 'plumbing_parts_reviews_get_markup' ) ) {
	function plumbing_parts_reviews_get_markup($field, $value, $editable=false, $clear=false, $snippets=false) {
		...
		$output .= '</div>';
		$output .= isset($field['accept']) && $field['accept'] && function_exists('plumbing_parts_sc_button') ? '<div class="reviews_accept">'.plumbing_parts_sc_button(array(), esc_html__('Accept your votes', 'plumbing-parts')).'</div>' : '';
