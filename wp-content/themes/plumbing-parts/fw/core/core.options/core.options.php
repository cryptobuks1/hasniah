...
if ( !function_exists( 'plumbing_parts_options_load_scripts' ) ) {
	function plumbing_parts_options_load_scripts() {
		...
		wp_enqueue_style( 'plumbing_parts-fontello-admin-style',	plumbing_parts_get_file_url('css/fontello-admin/css/fontello-admin.css'), array(), null);
		wp_enqueue_style( 'plumbing_parts-fontello-style', 			plumbing_parts_get_file_url('css/fontello/css/fontello.css'), array(), null);
		wp_enqueue_style( 'plumbing_parts-fontello-animation-style',plumbing_parts_get_file_url('css/fontello-admin/css/animation.css'), array(), null);
		// Plumbing Parts options styles
		wp_enqueue_style('plumbing_parts-options-style',			plumbing_parts_get_file_url('core/core.options/css/core.options.css'), array(), null);
		wp_enqueue_style('plumbing_parts-options-datepicker-style',	plumbing_parts_get_file_url('core/core.options/css/core.options-datepicker.css'), array(), null);
		...
		//} else if (plumbing_parts_storage_get('to_colorpicker') == 'tiny') {
			wp_enqueue_script('plumbing_parts-colors-script',		plumbing_parts_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );
			//wp_enqueue_style( 'plumbing_parts-colorpicker-style',	plumbing_parts_get_file_url('js/colorpicker/jqColorPicker.css'), array(), null);
			wp_enqueue_script('plumbing_parts-colorpicker-script',	plumbing_parts_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );
		//}

		// Input masks for text fields
		wp_enqueue_script( 'jquery-input-mask',				plumbing_parts_get_file_url('core/core.options/js/jquery.maskedinput.1.3.1.min.js'), array('jquery'), null, true );
		// Plumbing Parts core scripts
		wp_enqueue_script( 'plumbing_parts-core-utils-script',		plumbing_parts_get_file_url('js/core.utils.js'), array(), null, true );
		// Plumbing Parts options scripts
		wp_enqueue_script( 'plumbing_parts-options-script',			plumbing_parts_get_file_url('core/core.options/js/core.options.js'), array('jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-accordion', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-datepicker'), null, true );
		wp_enqueue_script( 'plumbing_parts-options-custom-script',	plumbing_parts_get_file_url('core/core.options/js/core.options-custom.js'), array('plumbing_parts-options-script'), null, true );
		...
		if (is_array($field['options']) && count($field['options']) > 0) {
			foreach ($field['options'] as $key => $item) {
				...
				if (in_array($field['type'], array('list', 'select', 'fonts')))
					...
					//$list .= '<img src="'.esc_attr($item).'" data-icon="'.esc_attr($key).'" alt="img" class="plumbing_parts_options_input_image" />';
			...
			case 'images':
				...
				if (empty($field['style']) || $field['style']=='select') {
					echo '<div class="plumbing_parts_options_caption_image iconadmin-down-open">'
						//.'<img src="'.esc_url($caption).'" alt="img" />'
...
if ( !function_exists( 'plumbing_parts_options_save' ) ) {
	...
	function plumbing_parts_options_save() {
		...
		if ($mode == 'save') {
			...
			parse_str($_POST['data'], $post_data);
			if (!plumbing_parts_storage_empty('post_override_options', 'fields')) {
				$options = plumbing_parts_array_merge(plumbing_parts_storage_get('options'), plumbing_parts_storage_get_array('post_override_options', 'fields'));
			}
		} else
			$post_data = array();
